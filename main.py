...
plots_folder = config["plots_directory"]
ext = config["plots_extension"]
...
if config["duration"] != 0:
    ...
    plots_folder = plots_folder  + str(config["duration"]) + '_s' + '\\'
    os.makedirs(plots_folder, exist_ok=True)
    # print(f"Folder ready: {plots_folder}")
    filename = "raw_ecg_" + str(config["duration"]) + '_s' + ext
else:
...
if config["duration"] != 0:
    filename = "ecg_frequency_spectrum_" + str(config["duration"]) + '_s' + ext
    plt.savefig(plots_folder + filename)
else:
    filename = "ecg_frequency_spectrum" + ext
    plt.savefig(plots_folder + filename)
...
filtered_ecg = bandpass_filter(
    ecg_segment,
    fs,
    lowcut =filter_config["lowcut_hz"],
    highcut=filter_config["highcut_hz"],
...
if config["duration"] != 0:
    filename = "ecg_raw_vs_filtered_ecg" + str(config["duration"]) + '_s' + ext
    plt.savefig(plots_folder + filename)
else:
    filename = "ecg_raw_vs_filtered_ecg" + ext
    plt.savefig(plots_folder + filename)
...
r_peaks = detect_r_peaks(
    filtered_ecg,
    fs,
    min_distance_seconds=peak_config["min_distance_seconds"],
...
if config["duration"] != 0:
    filename = "ecg_r_peaks_detected_" + str(config["duration"]) + '_s' + ext
    plt.savefig(plots_folder + filename)
else:
    filename = "ecg_r_peaks_detected" + ext
    plt.savefig(plots_folder + filename)
...
#plt.show()
#plt.savefig('C:\\git\\ecg-heart-rate-algorithm\\docs\\images\\instantaneous_heart_rate.png')
if config["duration"] != 0:
    filename = "ecg_instantaneous_heart_rate_" + str(config["duration"]) + '_s' + ext
    plt.savefig(plots_folder + filename)
else:
    filename = "ecg_instantaneous_heart_rate" + ext
    plt.savefig(plots_folder + filename)
...
#reference_samples, reference_symbols = \
#    load_reference_annotations("100")
...
tp, fp, fn, timing_errors, false_positive_peaks = evaluate_detection(
    ...
    reference_peaks,
    fs,
    tolerance_ms=validation_config["tolerance_ms"]
)
...
if len(false_positive_peaks) > 0:
    ...
    #plt.show()
    #plt.savefig('C:\\git\\ecg-heart-rate-algorithm\\docs\\images\\false_positive_analysis.png')
    if config["duration"] != 0:
        filename = "false_positive_analysis" + str(config["duration"]) + '_s' + ext
        plt.savefig(plots_folder + filename)
    else:
        filename = "false_positive_analysis" + ext
        plt.savefig(plots_folder + filename)
